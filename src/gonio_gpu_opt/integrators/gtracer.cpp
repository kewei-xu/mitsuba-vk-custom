...
public:
    ...
    GonioTracerIntegrator(const Properties &props) : Base(props) {
        ...
        m_coalesce = props.get<bool>("coalesce", false);
    ...
    TensorXf render(Scene *scene,
        ...
        uint32_t samples_per_pass = total_samples;
        if constexpr (dr::is_jit_v<Float>) {
            constexpr uint32_t default_wavefront_size = 262144u;
            if (m_samples_per_pass == (uint32_t) -1) {
                samples_per_pass = std::min(default_wavefront_size, total_samples);
                while (samples_per_pass > 1 && (total_samples % samples_per_pass) != 0)
                    --samples_per_pass;
            } else {
                samples_per_pass = std::min(m_samples_per_pass, total_samples);
            }
        } else {
            samples_per_pass = (m_samples_per_pass == (uint32_t) -1)
                ? total_samples
        ...
        if constexpr (!dr::is_jit_v<Float>) {
            ...
            splat_surface_hits(film, gonio_sensor, surface_hits_total);
            if (develop)
                result = film->develop();
        } else {
            ...
            ref<ImageBlock> block =
                film->create_block(ScalarVector2u(0), false, false);
            block->set_offset(film->crop_offset());
            block->set_coalesce(m_coalesce);

            Timer timer;
            for (uint32_t i = 0; i < n_passes; ++i) {
                surface_hits_total += sample(scene, gonio_sensor, sampler, block.get());
    ...
    size_t sample(const Scene *scene,
        ...
        active &= pi.is_valid();
        size_t surface_hits = (size_t) dr::slice(dr::count(active));

        trace_light_ray(scene, gonio_sensor, sampler, ray, throughput, pi, block, active);
        return surface_hits;
    }
    ...
    void splat_surface_hits(Film *film,
                            const GonioSensor<Float, Spectrum> *sensor,
                            size_t surface_hits_total) const {
        if (surface_hits_total == 0 || sensor->header_cells() == 0)
            return;
        ...
        Float count = (Float) (ScalarFloat) surface_hits_total;
        Mask active = true;
        if (block->channel_count() == 5) {
            Float values[5] = { 0.f, 0.f, 0.f, 0.f, count };
            block->put(pos, values, active);
        } else {
            Float values[4] = { 0.f, 0.f, 0.f, count };
            block->put(pos, values, active);
        }
        film->put_block(block);
    }
